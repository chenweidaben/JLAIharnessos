/**
 * 健澜科技数智医院智能体 - integration/adapters/his/LianzhongHISAdapter.ts
 *
 * Copyright (c) 2026 健澜科技. All rights reserved.
 */

/**
 * 健澜科技数智医院智能体 - 联众智慧HIS适配器（骨架）
 *
 * 版权所有 (c) 2026 健澜科技
 * 本文件为健澜科技专有技术文档，未经授权不得复制、传播或用于其他用途。
 *
 * 北京联众智慧（Lizong）HIS系统适配器骨架。
 * 联众智慧在区域医疗与基层医疗市场有较高占有率，
 * 集成以 REST API + WebService 为主。
 *
 * 本文件为骨架实现，包含认证流程、字段映射配置与全部接口方法骨架。
 *
 * @module integration/adapters/his/LianzhongHISAdapter
 */
...
/** 联众HIS适配器配置 */
export interface LianzhongHISConfig extends Partial<AdapterConfig> {
  appId?: string;
  appSecret?: string;
  orgCode?: string;
}
 ...
 * TODO: 完成实际对接。
 */
export class LianzhongHISAdapter extends BaseAdapter implements HISAdapter {
  private patientEventListeners = new Set<(event: PatientEvent) => void>();
  private orderEventListeners = new Set<(event: OrderEvent) => void>();

  constructor(config: LianzhongHISConfig = {}) {
    const adapterConfig = createDefaultAdapterConfig({
      id: config.id ?? 'his-lianzhong',
      type: 'his',
      vendor: '联众智慧',
      version: config.version ?? 'LZ 6.0',
      name: config.name ?? '联众HIS适配器',
      description: '对接联众智慧HIS系统',
      endpoint: config.endpoint ?? 'https://his-api.hospital.com/lz',
      timeout: config.timeout ?? 30000,
      protocol: 'rest',
      auth: {
        type: 'apiKey',
        apiKey: config.appId,
        apiKeyHeader: 'X-LZ-AppKey',
        ...config.auth,
      },
      fieldMapping: {
        patient: [
          { sourceField: 'blh', targetField: 'patientId' },
          { sourceField: 'xm', targetField: 'name' },
          { sourceField: 'xb', targetField: 'gender', valueMap: { '1': 'male', '2': 'female' } },
        ],
      },
      ...config,
    });
  ...
  private notImpl(method: string, detail = ''): Promise<never> {
    return Promise.reject(
      new AdapterError(
        ...
        `[LianzhongHISAdapter] ${method} 尚未实现${detail ? `（${detail}）` : ''}`,
