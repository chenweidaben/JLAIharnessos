# medical_record_qa — 病历智能问答

> 所属分类：**电子病历** ｜ 工具名：`medical_record_qa` ｜ 风险等级：**LOW**（低（只读/查询类））

## 工具描述

针对指定病历进行自然语言问答，基于病历上下文回答与诊疗相关的问题。
...
{
  "success": true,
  "data": { "tool": "medical_record_qa", "status": "ok" }
}
```
