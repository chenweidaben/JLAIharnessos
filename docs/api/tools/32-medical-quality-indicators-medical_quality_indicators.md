# medical_quality_indicators — 医疗质量指标

> 所属分类：**运营管理** ｜ 工具名：`medical_quality_indicators` ｜ 风险等级：**LOW**（低（只读/查询类））

## 工具描述

汇总 DRG、合理用药、平均住院日、非计划重返等医疗质量关键指标。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| indicators[] | array | 指标（名称/数值/目标值/达标） |
...
| 错误码 | 含义 |
| --- | --- |
| PERMISSION_DENIED | 缺少 quality:read 权限 |
...
{
  "success": true,
  "data": { "tool": "medical_quality_indicators", "status": "ok" }
}
```
