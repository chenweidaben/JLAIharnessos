# get_lab_result — 获取检验结果

> 所属分类：**检验检查** ｜ 工具名：`get_lab_result` ｜ 风险等级：**LOW**（低（只读/查询类））

## 工具描述

查询患者检验报告结果，含参考区间与异常标记，支持按项目过滤。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| results[] | array | 检验结果（项目/结果/单位/参考区间/异常标志） |
...
{
  "success": true,
  "data": { "tool": "get_lab_result", "status": "ok" }
}
```
