# medical_record_quality_check — 病历质量检查

> 所属分类：**质控管理** ｜ 工具名：`medical_record_quality_check` ｜ 风险等级：**LOW**（低（只读/查询类））

## 工具描述

对病历运行环节进行内涵质控，输出缺陷项与扣分，辅助医生实时整改。
...
{
  "success": true,
  "data": { "tool": "medical_record_quality_check", "status": "ok" }
}
```
