# get_prescription_list — 查询处方列表

> 所属分类：**处方药品** ｜ 工具名：`get_prescription_list` ｜ 风险等级：**LOW**（低（只读/查询类））

## 工具描述

按患者/状态查询处方列表，供医生与药师查看。
...
| 错误码 | 含义 |
| --- | --- |
| PERMISSION_DENIED | 缺少 prescription:read 权限 |
...
{
  "success": true,
  "data": { "tool": "get_prescription_list", "status": "ok" }
}
```
