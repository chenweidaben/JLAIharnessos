# follow_up_management — 随访管理

> 所属分类：**患者服务** ｜ 工具名：`follow_up_management` ｜ 风险等级：**LOW**（低（只读/查询类））
...
| 所需权限 | `followup:read`, `followup:write` |
| 适用角色 | doctor, nurse |
| 是否只读 | 是 |
...
| 错误码 | 含义 |
| --- | --- |
| PERMISSION_DENIED | 缺少 followup 权限 |
...
{
  "success": true,
  "data": { "tool": "follow_up_management", "status": "ok" }
}
```
