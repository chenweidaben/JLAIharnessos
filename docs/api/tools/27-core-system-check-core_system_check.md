# core_system_check — 核心制度落实检查

> 所属分类：**质控管理** ｜ 工具名：`core_system_check` ｜ 风险等级：**LOW**（低（只读/查询类））

## 工具描述

检查三级查房、查对、交接班、危急值等医疗核心制度落实情况。
...
{
  "success": true,
  "data": { "tool": "core_system_check", "status": "ok" }
}
```
