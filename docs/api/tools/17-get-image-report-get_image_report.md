# get_image_report — 获取影像报告

> 所属分类：**检验检查** ｜ 工具名：`get_image_report` ｜ 风险等级：**LOW**（低（只读/查询类））
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| reports[] | array | 影像报告（部位/结论/建议/报告医生） |
...
| 错误码 | 含义 |
| --- | --- |
| PERMISSION_DENIED | 缺少 imaging:read 权限 |
...
{
  "success": true,
  "data": { "tool": "get_image_report", "status": "ok" }
}
```
