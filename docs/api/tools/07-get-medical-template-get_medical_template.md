# get_medical_template — 获取病历模板

> 所属分类：**电子病历** ｜ 工具名：`get_medical_template` ｜ 风险等级：**LOW**（低（只读/查询类））

## 工具描述

查询并获取按科室/类型分类的病历书写模板。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| templates[] | array | 模板列表（模板ID/名称/正文） |
...
{
  "success": true,
  "data": { "tool": "get_medical_template", "status": "ok" }
}
```
