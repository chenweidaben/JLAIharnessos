# get_order_list — 查询医嘱列表

> 所属分类：**医嘱管理** ｜ 工具名：`get_order_list` ｜ 风险等级：**LOW**（低（只读/查询类））

## 工具描述

按患者/就诊查询已开立医嘱，支持按医嘱状态与类型过滤。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| orders[] | array | 医嘱列表（ID/内容/状态/开立人/时间） |
...
| 错误码 | 含义 |
| --- | --- |
| PERMISSION_DENIED | 缺少 order:read 权限 |
...
{
  "success": true,
  "data": { "tool": "get_order_list", "status": "ok" }
}
```
