# treatment_plan_suggestion — 治疗方案建议

> 所属分类：**临床决策支持** ｜ 工具名：`treatment_plan_suggestion` ｜ 风险等级：**LOW**（低（只读/查询类））

## 工具描述

基于诊断与指南知识库，推荐治疗方案、用药路径与注意事项。
...
{
  "success": true,
  "data": { "tool": "treatment_plan_suggestion", "status": "ok" }
}
```
