...
> 所属分类：**临床决策支持** ｜ 工具名：`diagnosis_suggestion` ｜ 风险等级：**LOW**（低（只读/查询类））

## 工具描述

基于患者症状、体征、检验检查结果，由 CDS 引擎给出可能诊断鉴别列表与置信度。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| suggestions[] | array | 鉴别诊断（诊断/置信度/依据） |
...
{
  "success": true,
  "data": { "tool": "diagnosis_suggestion", "status": "ok" }
}
```
